...
collection = chroma_client.get_or_create_collection("mvp_tour_rag")
collection.add(
    documents=[
...
# [도구3] RAG 검색 함수
@tool(approval_mode="never_require")
def rag_search(query: Annotated[str, Field(
        description="사용자의 질문"
    )]) -> str:
    """ChromaDB를 사용하여 RAG 검색을 수행하는 도구 함수입니다."""
    ...
    if results and results['documents'] and results['documents'][0]:
        ...
        print(f"Tool rag_search called with query: {query}. Returning: {answer}")
        return answer
    else:
        print(f"Tool rag_search called with query: {query}. No results found.")
...
async def main() -> None:
    ...
    agent = OpenAIChatClient(
        ...
        """,
        tools=[get_weather, get_exchange_money, rag_search],
    )
